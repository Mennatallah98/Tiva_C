...
//Mode functions
void GPIODirModeSet(gpio_port port, UC pins, gpio_mode Mode, UC af_fn)
{
    //AF
    ...
    *reg &= ~(pins);
    ...
    {
        SET_BITS(reg,pins);
        ADDRESS reg = GPIOSetAddress(port,GPIOPCTL);
        UC i,bit;
        for(i=0;i<8;i++)
        {
           bit = ((pins>>i)&1);
...
{
    ADDRESS reg_M = GPIOSetAddress(port, GPIOIM);
    ADDRESS reg_S = GPIOSetAddress(port, GPIOIS);
    ADDRESS reg_E = GPIOSetAddress(port, GPIOIBE);
    ADDRESS reg_T = GPIOSetAddress(port, GPIOIEV);
    ADDRESS reg_C = GPIOSetAddress(port, GPIOICR);
...
{
    ADDRESS reg = GPIOSetAddress(port, GPIOIS);
    return GET_BITS(reg,pins);
}

UC GPIOGetINTBothedges(gpio_port port, UC pins)
{
    ADDRESS reg = GPIOSetAddress(port, GPIOIBE);
    return GET_BITS(reg,pins);
}

UC GPIOGetINTTrigger(gpio_port port, UC pins)
{
    ADDRESS reg = GPIOSetAddress(port, GPIOIEV);
    return GET_BITS(reg,pins);
}

UC GPIOGetINTTrMask(gpio_port port, UC pins)
{
    ADDRESS reg = GPIOSetAddress(port, GPIOIM);
    return GET_BITS(reg,pins);
}

UC GPIOGetINTTrOcuurance(gpio_port port, UC pins)
{
    ADDRESS reg = GPIOSetAddress(port, GPIORIS);
    return GET_BITS(reg,pins);
}

UC GPIOGetINTTrStatus(gpio_port port, UC pins)
{
    ADDRESS reg = GPIOSetAddress(port, GPIOMIS);
    return GET_BITS(reg,pins);
}
//********************************************************************************
